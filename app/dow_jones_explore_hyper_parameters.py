...
hyper_parameter_folder = os.path.join(parent_folder, 'hyper_parameter_test', classifier, data_preprocessing)

file_name_list = os.listdir(hyper_parameter_folder)
hyper_parameter_avg_dict = collections.defaultdict(lambda :{})
hyper_parameter_best_value_dict = collections.defaultdict(lambda :{})
...
for file_name in file_name_list:
    ...
    with open (file_path, 'r') as f:
        ...
        regressor_eva_metrics_best_value_list = [min(regressor_eva_metrics_list[0]), # loss
                                                 max(regressor_eva_metrics_list[1]), # iteration step
                                                 min(regressor_eva_metrics_list[2]), # rmse
                                                 max(regressor_eva_metrics_list[3]),  # avg_pc
                                                 max(regressor_eva_metrics_list[4])]  # hit

    hyper_parameter_avg_dict[hyper_parameter_tuple] = regressor_eva_metrics_avg_list
    hyper_parameter_best_value_dict[hyper_parameter_tuple] = regressor_eva_metrics_best_value_list
# ----------------------------------------------------------------------------------------------------------------------
...
# ----------------------------------------------------------------------------------------------------------------------
# sort by average
hyper_parameter_result_save_folder = os.path.join(parent_folder, 'hyper_parameter_test', classifier, 'RESULT')
#
...
def save_hyper_parameter_result_to_file_regressor(hyper_parameter_list, save_path):
...
# ----------------------------------------------------------------------------------------------------------------------
hyper_parameter_avg_list = list(hyper_parameter_best_value_dict.items())

# average
# (1.) sort by rmse
hyper_parameter_best_list_sort_by_rmse = sorted(hyper_parameter_avg_list, key = lambda x:x[1][2])
save_hyper_parameter_result_to_file_regressor(hyper_parameter_best_list_sort_by_rmse, hyper_parameter_best_rmse_save_path)

# (2.) sort by avg_pc
hyper_parameter_best_list_sort_by_avg_pc = sorted(hyper_parameter_avg_list, key = lambda x:x[1][3], reverse=True)
save_hyper_parameter_result_to_file_regressor(hyper_parameter_best_list_sort_by_avg_pc, hyper_parameter_best_avg_pc_save_path)

# (3.) sort by hit
hyper_parameter_best_list_sort_by_hit = sorted(hyper_parameter_avg_list, key = lambda x:x[1][4], reverse=True)
save_hyper_parameter_result_to_file_regressor(hyper_parameter_best_list_sort_by_hit, hyper_parameter_best_hit_save_path)
# ----------------------------------------------------------------------------------------------------------------------
